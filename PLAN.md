...
- **行政體系 API**：新增中央與地方各部會/局處等行政組織結構資訊，並收錄各機關首長名單。目前 ccapi 只有「議會」（立法/監督端），完全沒有「行政」（執行端）資料，兩者是台灣地方自治的兩個對應面向
  - **目標**：整理出各縣市有哪些一級機關（局處）、二級機關（科室）的完整清單
  - **現況（2026-09-08 盤點）：目前沒有任何一份現成來源可以直接拿來用，三個候選來源都有明顯缺口**：
    - `tw.gov.moda~ref~oid`（`https://data.openfun.tw/datasets/tw.gov.moda~ref~oid/skill.md`）：中央機關為主，地方局處覆蓋不完整；立法院多數下屬單位甚至完全沒有 OID
    - `tw.gov.dgpa~ref~gov-org`（`https://data.openfun.tw/datasets/tw.gov.dgpa~ref~gov-org/skill.md`）：約 17,388 筆，但大部分是歷史裁撤機關；**只有六都有完整的機關列表，非六都很多縣市完全沒有局處代碼**；且沒有司處級以下的單位代碼
    - 兩者都是中央政府角度整理的名冊，不是「以縣市為單位、逐一盤點局處」的設計，這是非六都缺口的根本原因
  - **下一步方向：改從預算書下手**——地方政府總預算書逐年都會列出所有「預算機關」（局處，甚至到科室層級的用途別/子計畫），理論上可以逐年逐縣市把預算書裡出現過的機關名稱都撈出來，反推組織架構清單，覆蓋率應該比前兩個來源好很多，尤其是非六都
    - 參考 `https://budget.openfun.app/skill.md`——這是本機已有、涵蓋中央+22縣市總預算書的解析平台（歲出/歲入/基金，PDF 解析成結構化資料存在 Elasticsearch），已有「預算機關」欄位可查，但**目前這個系統是預算導向、不是組織結構導向**，沒有現成的「機關清單」端點，需要另外寫查詢/彙整邏輯把各縣市歷年出現過的機關名稱去重整理成清單
    - 已知限制：舊年度PDF多為掃描檔沒有文字層、部分縣市網站有技術障礙沒抓到、基金分支明細只有臺南市/新竹市做得完整——用預算書重建機關清單一樣會遇到「覆蓋率因縣市而異」的問題，需要跟逐字稿/議案這些既有資料集一樣，先盤點各縣市實際涵蓋範圍再決定怎麼處理缺口
  - 這一項還沒開始動工，先记录候選資料來源盤點結果，之後排進 Phase 再細部規劃
...
- [x] **會期（Session）／場次（Sitting）**（2026-08-26）：都補上「來源網址」了（各議會
  ...
  對這種連結會回 404（疑似 ASP.NET WebForms 站台需要 session/postback 狀態），格式本身
  正常，只是沒辦法用程式自動驗證每一筆都能開。已更新 `import-session.php`／
  `import-sitting.php` 吃進這個欄位並重新匯入，目前還沒加到網頁上顯示連結
- [ ] **委員會（Committee）**：來源 `data.csv` 目前還是**完全沒有任何來源追溯欄位**（無
  官方網址、無檔案/頁碼）
