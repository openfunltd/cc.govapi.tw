...
- [ ] **議員（Councilor）**：來源 `議員.jsonl`（moi 地方公職人員資訊專區）目前只有照片是
  ...
  不需要額外處理
- [ ] **候選人（Candidate）／議案（Bill）**：這兩個已經有 `來源PDF`／`來源檔案` +
  `來源頁碼`（檔案內來源已經算完整），但都**沒有官方來源網址**。候選人公報應該來自中選會
  「選舉公報」查詢系統，議案應該來自各議會自己的議事錄公告頁，確認這兩個系統有沒有穩定
  的查詢網址規則（例如用選舉代碼/年份組出網址），可以直接請上游提供 URL pattern 或範例
  網址，不一定需要每筆資料都存一次完整網址
