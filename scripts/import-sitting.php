 ...
 *
 * 來源：場次.csv（欄位：代碼, 會期代碼, 日期, 星期, 時段, 會次, 場次類別, 委員會名稱, 議程說明, 停會原因, 開始時間, 結束時間, 來源網址）
 * 所有來源欄位直接沿用原始名稱匯入 ES
 * 來源網址是該議會官網的公告/議事日程頁連結（官方來源，不是逐頁的檔案內來源），涵蓋率
 * 約 99.99%（37898 筆裡 5 筆缺值）；同一會期底下的場次常常共用同一個連結（該議會的
 * 議事日程列表頁，不是每場次各自的深連結），實測部分議會網站對這種連結會回 404（疑似
 * ASP.NET WebForms 站台需要 session/postback 狀態，直接開網址進不去），格式本身看
 * 起來正常，只是沒辦法用程式自動驗證每一筆都能開
...
$known_source_keys = [
    '代碼', '會期代碼', '日期', '星期', '時段', '會次',
    '場次類別', '委員會名稱', '議程說明', '停會原因', '開始時間', '結束時間', '來源網址',
];
