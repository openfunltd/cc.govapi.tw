...
class CCAPI_Type
{
    ...
    /**
     * run 依照 $type 執行對應的 $method，Ex: CCAPI_Type::run('user', 'get', [1]) => CCAPI_Type_User::get(1)
     */
    public static function run($type, $method, $args = [])
    {
        $class = 'CCAPI_Type_' . CCAPI_Helper::ucfirst($type);
    ...
    {
        return strtolower(str_replace('CCAPI_Type_', '', get_called_class())) . 's';
    }
    ...
    public static function termSearch($class, $field, $type, $config)
    {
        ...
        if (!$query_field) {
            $query_field = CCAPI_Type::run($type, 'reverseField', [$config[0]]);
        }

        $output_field = CCAPI_Type::run($type, 'reverseField', [$config[1]]);
